...
export function ListingsHeader({
  ...
  const popularLocations: string[] = allLocations;
  ...
  useEffect(() => {
    const availableLocations = popularLocations.filter(
      (loc) => !selectedLocations.includes(loc)
    );

    if (searchQuery.trim()) {
      const matched = availableLocations.filter((loc) =>
        loc.toLowerCase().includes(searchQuery.toLowerCase())
      );
      setFilteredLocations(matched.slice(0, 6));
    } else {
  ...
  useEffect(() => {
    ...
    // Оновлюємо фільтр town
    if (selectedLocations.length > 0) {
      newFilters.town = selectedLocations[0]; // або всі: selectedLocations.join(",")
    } else {
      delete newFilters.town;
    }

    // Оновлюємо development_name
    if (searchQuery.trim()) {
      newFilters.development_name = searchQuery;
    } else {
      delete newFilters.development_name;
    }

    onFiltersChange(newFilters);
  }, [selectedLocations, searchQuery]);

  const addLocation = (e, location: string) => {
  ...
  const removeLocation = (location: string) => {
    ...
    updateFilter("town", newLocations);
  };
  ...
  const toggleBedroom = (bed: string) => {
    const newBeds = selectedBeds.includes(bed)
  ...
  const applyPriceFilter = () => {
    const minPrice = priceFrom
      ? Number.parseInt(priceFrom.replace(/,/g, ""))
      : null;
    const maxPrice = priceTo
      ? Number.parseInt(priceTo.replace(/,/g, ""))
      : null;
  ...
  return (
    <header className="bg-white border-b border-gray-100 relative sticky top-0 z-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-3">
        <div className="flex items-center justify-between">
          ...
          {showSearch && (
            <div
              ref={searchRef}
              className="relative w-[35%] min-w-[400px] max-w-[600px] hidden md:block"
            >
              <form onSubmit={handleSearch}>
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full ${
                      selectedLocations.length > 0 ? "pl-12" : "pl-12"
                  ...
                  <div className="py-2">
                    <div className="px-4 py-2 text-xs font-light text-gray-500 border-b border-gray-100 sticky top-0 bg-white">
                      ...
                      <span className="text-gray-400 ml-1">
                        (Select up to {MAX_LOCATIONS - selectedLocations.length}{" "}
                        more)
                      </span>
                    </div>
                    {filteredLocations.map((location, index) => (
                      <button
                        ...
                        onClick={(e) => addLocation(e, location)}
                      >
            ...
            <div className="hidden md:flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <input
                              ...
                              value={priceFrom}
                              onChange={(e) =>
                                setPriceFrom(formatPriceInput(e.target.value))
                              }
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={priceTo}
                              onChange={(e) =>
                                setPriceTo(formatPriceInput(e.target.value))
                              }
                              className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-1 focus:ring-gray-300 focus:border-gray-300 outline-none bg-white text-gray-900"
                            />
                          </div>
                ...
                <div className="flex items-center gap-2 border-l border-gray-200 pl-4 ml-2">
                  <button
                    onClick={() =>
                      handleCategoryChange("new-building", "new building")
                    }
                  ...
                  <button
                    onClick={() =>
                      handleCategoryChange("secondary", "secondary")
                    }
                    className={`px-3 py-1 rounded-lg text-[14px] font-light transition-all duration-200 ${
      ...
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 bg-[#5784FF]/20 z-30"
          onClick={() => setMobileMenuOpen(false)}
        />
      )}
